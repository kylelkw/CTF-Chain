...
const claimContent = `import { NextRequest, NextResponse } from "next/server";
import {
  createPublicClient,
  createWalletClient,
  http,
...
const poolContent = `import { NextRequest, NextResponse } from "next/server";
...
// Write files
fs.writeFileSync(path.join(claimPath, 'route.ts'), claimContent);
fs.writeFileSync(path.join(poolPath, 'route.ts'), poolContent);

console.log(`✓ Created: ${path.join(claimPath, 'route.ts')}`);
console.log(`✓ Created: ${path.join(poolPath, 'route.ts')}`);
